...
@endphp
@section('title')
    @lang('site.reports_general_expenses')
@endsection
...
@section('content')
    ...
    <div class="card">
        <h5 class="card-header bg-success text-center">
            @lang('site.reports_general_expenses') <span id="total-number"></span>
        </h5>
        <div class="card-body">
            {{-- Date section --}}
            <div class="row date">
            ...
            <div class="card date textDirection table-card ">
                <div class="card-body" id="Table_se">
                    {{-- @php
                    $vAL =  filter_var($invoice->overall_total,FILTER_SANITIZE_NUMBER_FLOAT,FILTER_FLAG_ALLOW_FRACTION);
...
<script src="{{ asset('dist/js/prettify.js') }}"></script> --}}
    ...
    <script>
        function ExportToExcel(type, fn, dl) {
           var elt = document.getElementById('general_expenses_report');
        ...
        function printData() {
           var divToPrint =document.getElementById("general_expenses_report");
           newWin= window.open("");
        ...
        $(".eventBtu").click(function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $("#Table_se").html(data);
                    var priceEls = document.getElementsByClassName("total_left_not_rest");
                    var priceres = document.getElementsByClassName("total_left_rest");
                    var total_all_left = document.getElementsByClassName("total_all_left");
                    $total_left_not_rest = 0;
                    $total_left_rest = 0;
                    $total_all_left = 0;
                    for (var i = 0; i < priceEls.length; i++) {
                        var price = priceEls[i].innerText;
                        console.log(priceEls[i].innerText);
                        $total_left_not_rest += Math.floor(price);
                    }
                    $(".total_not_rest_left").text($total_left_not_rest);
                    for (var i = 0; i < priceres.length; i++) {
                        var price = priceres[i].innerText;
                        $total_left_rest += Math.floor(price);
                    }
                    $(".total_rest_left").text($total_left_rest);
                    for (var i = 0; i < total_all_left.length; i++) {
                        var price = total_all_left[i].innerText;
                        $total_all_left += Math.floor(price);
                    }
                    $(".total_all_left_result").text($total_all_left);
                }
            });
        });
    </script>
